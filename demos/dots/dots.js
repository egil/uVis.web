...
require(['nextTick', 'shims', 'uvis/Template', 'uvis/TemplateProperty', 'uvis/TemplateEvent'], function (nt, s, ut, up, ue) {
    $(function () {
        ...
        var inputColor = new ut.uvis.Template('inputColor', 'html#input', form, function () {
        ...
        var dot = new ut.uvis.Template('dot', 'html#span', form, function (t) {
            return t.walk().get('inputCount').event('change');
        });

        dot.properties.add('title', new up.uvis.ComputedTemplateProperty('title', function (c) {
            return Rx.Observable.returnValue(c.index);
        }));

        dot.properties.add('text', new up.uvis.ComputedTemplateProperty('text', function (c) {
            return Rx.Observable.returnValue(c.index);
        }));

        dot.properties.add('style', new up.uvis.ComputedTemplateProperty('style', function (c) {
            var sizeObs = c.template.walk().get('inputSize').event('change');

            return Rx.Observable.returnValue(c.index).select(function (i) {
                return c.template.walk().get('inputColor', i % 4).event('input');
